...
void topicCorpus::msgSampling()
{
  ...
  for (int c = 0; c < nConvs; c++)
  {
    ...
    for (int x = 0; x < (int) trainVotesPerConv[c].size(); x++)
    {
      ...
      { // Update topic counts if the topic for this message changed
        convTopicSums[cv]--;
        convTopicSums[cv]++;
      ...
      if (newDiscourse != oldDiscourse)
      { // Update discourse counts if the discourse for this message changed
      ...
      for (int wp = 0; wp < (int) vi->words.size(); wp++)
      {
        ...
        typeScores[1] = exp(typeSwitcher[newDiscourse][1] + bg_discourseWords[wi] + discourseWords[wi][newDiscourse]);
...
void topicCorpus::dl(double* grad)
{
  ...
  for (int u = 0; u < nUsers; u++)
  {
    ...
    for (int d = 0; d < D; d++)
    {
      double p = -lambda * (userDiscourseCounts[u][d] - userDiscourseSums[u] * exp(*kappa_disc * delta_user[u][d]) / dZ);
...
void topicCorpus::meanAveragePrecision(double& train, double& valid, double& test)
{
  ...
  for (map<int, vector<rating> >::iterator it = validRatings.begin(); it != validRatings.end(); it++)
  {
  ...
  for (map<int, vector<rating> >::iterator it = testRatings.begin(); it != testRatings.end(); it++)
  {
...
void topicCorpus::save(std::string modelPath, std::string resultPath, std::string scorePath, std::string learningPath)
{
  ...
  fprintf(f, "Topic num: %d, Discourse num: %d, topicProportion: %.3f, latentReg: %.3f, lambda: %.3f, c1: %d, c2: %d\n", K, D, topicProportion, latentReg, lambda, c1, c2);
  ...
  fprintf(f, "nDCG@5: %f, nDCG@10: %f, nDCG@25: %f, nDCG@50: %f\n\n", ndcgResult[0], ndcgResult[1], ndcgResult[2], ndcgResult[3]);
...
int main(int argc, char** argv)
{
  ...
  else
  {
    ...
    strStream << "_" << lambda << "_" << topicProportion << "_" << K << "_" << D << "_" << percentage << "_" << c1 << "_" << c2 << "_";
